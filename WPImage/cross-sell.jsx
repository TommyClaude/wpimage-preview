// cross-sell.jsx — "More plugins" page: cross-sell of other NinjaTeam plugins.
// Uses the WP design system (icon tiles from the WP glyph set + flat brand-tinted
// colors). No fabricated logos. Two layouts: compact list & WP-style cards.
...
/* ---------------- Install button (shared) ---------------- */
function InstallBtn({ state, onInstall, id, now, big }) {
  const sz = big ? '' : ' sm';
  if (state === 'activated')
    return <button className={'wp-btn tertiary' + sz} disabled style={{ color:'var(--fg-muted)', cursor:'default' }}><Ic name="check" cls="xs" />Activated</button>;
  if (state === 'installing')
    return <button className={'wp-btn secondary' + sz} disabled><span className="wp-spinner"></span>Installing…</button>;
  return <button className={'wp-btn secondary' + sz} onClick={()=>onInstall(id)}><Ic name="cloud-download" cls="xs" />{now ? 'Install Now' : 'Install'}</button>;
}
...
/* Subtly-striped placeholder with a monospace caption naming what belongs here.
   Swap `src` (or replace the whole node) with a real company photo when ready. */
...
function AboutSection() {
  return (
    <div className="wp-card" style={{ marginBottom:24, overflow:'hidden' }}>
      <div style={{ display:'grid', gridTemplateColumns:'1.1fr 1fr', gap:0, alignItems:'stretch' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gridTemplateRows:'1fr 1fr', gap:8, padding:8, minHeight:340, background:'var(--surface-subtle)' }}>
...
function PluginsPage({ fireToast, layout = 'list', about = true }) {
  ...
  const head = (
    <React.Fragment>
      ...
      <div className="wp-tabs" style={{ marginTop:20 }}>
        {NT_CATS.map(c => (
          <div key={c.id} className={'wp-tab' + (cat===c.id ? ' active' : '')} onClick={()=>setCat(c.id)}>{c.label}</div>
        ))}
      </div>
    </React.Fragment>
  );
  ...
  return (
    <div style={{ padding:'28px 32px 64px', maxWidth:1080, margin:'0 auto' }}>
      ...
      <div className="wp-card">
        ...
        <div style={{ padding:'4px 28px 20px', display:'grid', gridTemplateColumns:'1fr 1fr', columnGap:40, rowGap:0 }}>
        ...
        <div style={{ padding:'16px 28px', borderTop:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
          ...
          <button className="wp-btn link" style={{ fontSize:13 }}>Browse all NinjaTeam plugins<Ic name="external" cls="xs" /></button>
        </div>
      </div>
    </div>
  );
