// components.jsx — shared UI for the WPImage dashboard (forked from the
// NinjaTeam WordPress plugin admin UI kit). All style objects are uniquely
...
function Toggle({ on, onChange, disabled }) {
  return (
    <span className={'wp-toggle' + (on ? ' on' : '')}
      role="switch" aria-checked={on} tabIndex={disabled ? -1 : 0}
      style={disabled ? { opacity:.5, pointerEvents:'none' } : null}
...
function SectionCard({ title, sub, icon, action, children, pad = true, style, bodyStyle }) {
  return (
    <div className="wp-card" style={{ marginBottom:20, ...style }}>
      {title && (
